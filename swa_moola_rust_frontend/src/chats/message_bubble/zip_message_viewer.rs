...
pub fn SecureZip(
    ...
    let zip_src_resource = LocalResource::new(move || {
    ...
    on_cleanup(move || {
        if let Some(Some(url)) = zip_src_resource.get_untracked() {
    ...
    view! {
        <Suspense fallback=move || view! { <div class="text-xs text-gray-400"> "Loading video..." </div> }>
            {move || zip_src_resource.get().map(|maybe_src| {
                if let Some(src) = maybe_src {
                    view! {
                        <a href=src target="_blank" class="flex items-center gap-3 p-2.5 bg-amber-50 hover:bg-amber-100/70 rounded-lg border border-amber-100 transition-colors max-w-xs text-xs">
                            <div class="p-2 bg-amber-500 rounded text-white font-bold text-[10px] tracking-wider uppercase shadow-sm">
                                "ZIP"
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-gray-800 font-medium truncate">{name.clone()}</p>
                                <p class="text-gray-400 text-[10px] mt-0.5">"Archive file Click to save"</p>
                            </div>
                        </a>
                    }.into_any()
                } else {
                    view! {
                         <div class="text-xs text-red-500"> "Failed to display Zip" </div> 
                    }.into_any()
                }
            })}
        </Suspense>
