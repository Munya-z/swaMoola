...
use crate::chats::models::{Conversation, ConversationParticipant, Message, AddParticipantPayload};
...
pub async fn get_conversation_messages(
    ...
    let messages = query.fetch_all(&pool) 
...
pub async fn get_conversation_participants(
    ...
    let participants = query.fetch_all(&pool) 
    .await.map_err(|e: sqlx::Error| {
        println!("Database query error: {}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
...
pub async fn add_new_participant_in_conversation(
    ...
    let query = sqlx::query_as!(
     ...
     let new_participant = query.fetch_one(&pool) 
    .await.map_err(|e: sqlx::Error| {
        println!("Database query error: {}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
